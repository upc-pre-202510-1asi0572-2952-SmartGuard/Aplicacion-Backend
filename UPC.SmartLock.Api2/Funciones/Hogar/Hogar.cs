...
namespace UPC.SmartLock.Api2.Funciones.Hogar
{
    public class Hogar : FunctionBase
    {
        ...
        public async Task<IActionResult> ActualizarHogarMysql(
           [HttpTrigger(AuthorizationLevel.Anonymous, "put", Route = "v1/hogarMysql")] HttpRequest req,
           ILogger log)
        {
            try
            {
                ...
                var blComercio = new HomeManager(repositorio);
                await blComercio.ActualizarHogar(hogarRequest);
                return FunctionBaseHttpMensaje.ResultadoOk();
            }
            catch (MensajeException mx)
            {
        ...
        [Function("EliminarHogarPorIdMysql")]
        public async Task<IActionResult> EliminarHogarPorIdMysql(
         [HttpTrigger(AuthorizationLevel.Anonymous, "delete", Route = "v1/hogarMysql/{hogarId}")] HttpRequest req,
         string hogarId,
         ILogger log)
        {
            {
                try
                {
                    ...
                    await blComercio.EliminarHogarPorId(hogarId);
        ...
        [Function("ObtenerHogaresPorPropietarioNicknameMysql")]
        public async Task<IActionResult> ObtenerHogaresPorPropietarioNicknameMysql(
            [HttpTrigger(AuthorizationLevel.Anonymous, "get", Route = "v1/hogares/propietario/{nickname}")] HttpRequest req, 
            string nickname, 
            ILogger log)
        {
            {
                try
                {
                    var repositorio = new Repositorio(_repositorioUpc.CadenaConexion, _repositorioUpc.Almacenamiento);
                    var blComercio = new HomeManager(repositorio);
                    var hogares = await blComercio.ObtenerHogaresPorPropietarioNickname(nickname);
